...
{
    NSLog(@"[Live2D METAL] *** GenerateShaders ENTRY POINT ***");
    NSLog(@"[Live2D METAL] GenerateShaders: _shaderSets.GetSize() = %d", _shaderSets.GetSize());
    
    if (_shaderSets.GetSize() > 0)
    {
        NSLog(@"[Live2D METAL] GenerateShaders: Already initialized, returning");
        return;
    }
    ...
    //シェーダライブラリのロード（.metal）
    // For Swift package environment, load from bundle resources
    NSLog(@"[Live2D METAL] GenerateShaders: Attempting to load metallib from bundle");
    NSLog(@"[Live2D METAL] GenerateShaders: mainBundle path: %@", [[NSBundle mainBundle] bundlePath]);
    NSLog(@"[Live2D METAL] GenerateShaders: mainBundle resourcePath: %@", [[NSBundle mainBundle] resourcePath]);
    
    // Try to load from the Swift Package bundle resources
    NSURL *libraryURL = nil;
    
    // Try mainBundle first
    libraryURL = [[NSBundle mainBundle] URLForResource:@"MetalShaders" withExtension:@"metallib"];
    NSLog(@"[Live2D METAL] GenerateShaders: mainBundle URL (no subdirectory): %@", libraryURL);
    
    // If not found, try with subdirectory
    if (!libraryURL) {
        libraryURL = [[NSBundle mainBundle] URLForResource:@"MetalShaders" withExtension:@"metallib" subdirectory:@"FrameworkMetallibs"];
        NSLog(@"[Live2D METAL] GenerateShaders: mainBundle URL (with FrameworkMetallibs): %@", libraryURL);
    }
    
    // If not found, try all bundles
    if (!libraryURL) {
        NSArray *allBundles = [NSBundle allBundles];
        NSLog(@"[Live2D METAL] GenerateShaders: Found %lu bundles", (unsigned long)allBundles.count);
        for (NSBundle *bundle in allBundles) {
            libraryURL = [bundle URLForResource:@"MetalShaders" withExtension:@"metallib"];
            if (libraryURL) {
                NSLog(@"[Live2D METAL] GenerateShaders: Found in bundle: %@", [bundle bundlePath]);
                break;
            }
        }
    }
    
    // FALLBACK: Check environment variable for absolute path
    if (!libraryURL) {
        const char* envPath = std::getenv("ARIA_METALLIB_PATH");
        NSLog(@"[Live2D METAL] GenerateShaders: ARIA_METALLIB_PATH env var: %s", envPath ? envPath : "(not set)");
        if (envPath && strlen(envPath) > 0) {
            NSString* absolutePath = [NSString stringWithUTF8String:envPath];
            NSString* shaderPath = [absolutePath stringByAppendingPathComponent:@"MetalShaders.metallib"];
            NSLog(@"[Live2D METAL] GenerateShaders: Checking ARIA_METALLIB_PATH: %@", shaderPath);
            if ([[NSFileManager defaultManager] fileExistsAtPath:shaderPath]) {
                libraryURL = [NSURL fileURLWithPath:shaderPath];
                NSLog(@"[Live2D METAL] GenerateShaders: Found via ARIA_METALLIB_PATH: %@", libraryURL);
            } else {
                NSLog(@"[Live2D METAL] GenerateShaders: File not found at ARIA_METALLIB_PATH");
            }
        }
    }
    
    // FALLBACK: Try hardcoded dev path
    if (!libraryURL) {
        NSString* projectPath = @"/Volumes/T7Sheald/Aria/Resources/FrameworkMetallibs";
        NSString* shaderPath = [projectPath stringByAppendingPathComponent:@"MetalShaders.metallib"];
        NSLog(@"[Live2D METAL] GenerateShaders: Checking hardcoded dev path: %@", shaderPath);
        if ([[NSFileManager defaultManager] fileExistsAtPath:shaderPath]) {
            libraryURL = [NSURL fileURLWithPath:shaderPath];
            NSLog(@"[Live2D METAL] GenerateShaders: Found via hardcoded dev path: %@", libraryURL);
        } else {
            NSLog(@"[Live2D METAL] GenerateShaders: File not found at hardcoded dev path");
        }
    }
    
    if (!libraryURL) {
        NSLog(@"[Live2D METAL] GenerateShaders: ERROR: Could not find MetalShaders.metallib in any location");
        NSLog(@"[Live2D METAL] GenerateShaders: Suggestion: Set ARIA_METALLIB_PATH environment variable to directory containing MetalShaders.metallib");
        NSLog(@"[Live2D METAL] GenerateShaders: Default fallback path would be: /Volumes/T7Sheald/Aria/Resources/FrameworkMetallibs/MetalShaders.metallib");
        return;
    }
    
    NSLog(@"[Live2D METAL] GenerateShaders: Final URL: %@", libraryURL);
    
    NSError *error = nil;
    id<MTLLibrary> shaderLib = [device newLibraryWithURL:libraryURL error:&error];
    if(!shaderLib)
    {
        NSLog(@"[Live2D METAL] GenerateShaders: ERROR: Couldnt create a default shader library: %@", error.localizedDescription);
        // assert here because if the shader libary isn't loading, nothing good will happen
        return;
    }
    NSLog(@"[Live2D METAL] GenerateShaders: Shader library loaded successfully");
    ...
    // ブレンドモードの組み合わせ分作成
    {
        ...
        for(csmInt32 i = 0; i <= Core::csmColorBlendType_Color; i++)
        {
            ...
            for (csmInt32 j = start; j <= Core::csmAlphaBlendType_DisjointOver; j++)
            {
                ...
                // Skip blend shader generation for basic rendering
...
{
    NSLog(@"[Live2D] GenerateBlendShader called for %s/%s - skipping for basic rendering", vertShaderFileName.GetRawString(), fragShaderFileName.GetRawString());
    // Skip blend shader generation for now - use basic shaders only
    // This avoids the crash when blend shader metallibs are not available
...
{
    NSLog(@"[Live2D] LoadShaderProgramFromFile called with vert: %s, frag: %s - returning NULL for basic rendering", vertShaderFileName.GetRawString(), fragShaderFileName.GetRawString());
    // For basic rendering, return NULL to avoid blend shader loading issues
    // Basic shaders are loaded from the main metallib in GenerateShaders
    return NULL;
}
